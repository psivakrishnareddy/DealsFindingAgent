...
## Frontier model with inference time techiniques - multishot encoding by providing similar vector items as context from the
## dataset with price to predict the price
class FrontierAgent(Agent):
    ...
    def __init__(self, collection):
        ...
        self.model = SentenceTransformer("sentence-transformers/all-MiniLM-L6-v2") # This is what the HuggngFaceEmbeddings uses as that is a wrapper for langchain, we are not using langchain here
        self.log("Frontier Agent is ready") # sentenceTransformer will tokenize and do word embeddings that creates meaning and relation vectors
